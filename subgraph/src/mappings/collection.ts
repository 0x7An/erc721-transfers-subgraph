...
export function handleTransfer(event: Transfer): void {
  ...
  if (tokenCollection == null) {
    ...
    let previousNonERC721Collection = NonERC721Collection.load(
      collectionAddress
    );
    ...
    if (!isERC721Supported(contract)) {
      let newNonERC721Collection = new NonERC721Collection(collectionAddress);
    ...
    tokenCollection.name = collectionAddress.substring(
      collectionAddress.length - 4,
